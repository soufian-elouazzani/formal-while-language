...
(*
									...
									(evalA)              (evalA)
                                                                       -----------          -----------
                                                                        2-1 = 1              1+2 = 3
                                                                    ----------- SN_Assign ----------- SN_Assign
                                                                    SN i1 S1 S'           SN i2 S' S2
                                                                   (S'=[0;2])            (S'=[0;2])
                                                                ----------------------------------------- SN_Seq            ------------------------- SN_While_false
                                 (evalB)                         SN corps [1; 2] [0; 3]                                  evalB "i<>0" [0;3] = false
                             ---------------
...
Proof.
  ...
  {cbn. cbv [corps_boucle].
   Compute (evalB(Bnot(Beqnat Ir N0)) [1;6]).
   eapply SN_While_true.
   {cbn. reflexivity.}
   {eapply SN_Seq.
    + apply SN_Assign.
...
Proof.
  ...
  }
  {
    ...
    + eapply SN_Seq.
    - apply SN_Assign.
    - cbn. eapply SN_Seq. {apply SN_Assign.} {cbn. apply SN_Assign.}
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_While_false *) cond i1 s e
                   | (* SN_While_true *) cond i1 s s1 s2 e sn1 hrec_sn1 sn2 hrec_sn2 
...
Proof.
  ...
  {cbn.
   destruct b ; try (apply SN_If_true; [apply H | apply IHsn] ).
...
(*INVERSION -> ignora os casos que não são interessantes e pega só os casos úteis*)
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_If_true *) cond i1 i2 s s' e sn hrec_sn
                   | (* SN_If_false *) cond i1 i2 s s' e sn hrec_sn
                   | (* SN_While_false *) cond i s eB
                   | (* SN_While_true *)  cond i s s1 eB sn hrec_sn
                   ].
  - apply SN'_Skip.
  - apply SN'_Assign.
  - eapply SN'_Seq.
  ...
  - apply SN'_If_false.
    {apply e.}
    {apply hrec_sn.}
  - apply SN'_While_false. apply eB.
...
Definition corps_boucleR : rinstr := RSeq (RAssign Il (Amo Ir N1)) (RAssign Xl (Apl Xr N1)).
...
Proof.
  ...
  {
    cbn.
    Compute (evalB(Beqnat Ir N0) [0;7]).
...
Fixpoint repeat_while (i:rinstr) : winstr := 
    ...
    | Repeat i1 b   => Seq (repeat_while i1) (While (Bnot b) (repeat_while i1))
    end.
...
Definition P2_rw := repeat_while P2.

Lemma test_P2_pour_rw : SN P2_rw [2; 5] [0; 7].
Proof.
  unfold P2_rw. unfold P2. cbn. eapply SN_Seq.
  {
    eapply SN_Seq.
    +apply SN_Assign.
    +cbn. apply SN_Assign.
  ...
  {
    cbn. Compute (evalB(Bnot (Beqnat Ir N0)) [1;6]). eapply SN_While_true.
    + cbn. reflexivity.
    + eapply SN_Seq.
    -apply SN_Assign.
    -cbn. apply SN_Assign.
     
     +cbn. Compute (evalB(Bnot (Beqnat Ir N0)) [0;7]). eapply SN_While_false. cbn. reflexivity.
  }
Qed.
 ...
 *)
(*while (i != 0) -> i = i - 1*)
Definition P_w := While (Bnot (Beqnat Ir N0)) (Assign Il (Amo Ir N1)).
Definition P_wr := while_repeat P_w.

Lemma test_wr_concrete : SNr P_wr [2] [0].
Proof.
  unfold P_wr. cbn.
  apply SNr_If_true.
  {
...
Proof.
  ...
  {cbn. apply SNr_If_true.
   ...
   -eapply SNr_Repeat_false.
    --apply IHsn1.
    --cbn. rewrite H5. apply negb_negb.
    --apply H6.
   -subst. inversion H6. subst. apply SNr_Repeat_true.
    --apply IHsn1.
    --cbn. rewrite H5. apply negb_negb.
  }
...
Proof.
  ...
  {
    eapply SOS_again.
    +Compute (evalB(Bnot(Beqnat Ir (Aco 2))) [0; 0; 1]). apply SOS1_If_true. cbn. reflexivity.
    +eapply SOS_again.
